...
export default function LoginPage() {
  ...
  return (
    <main className="login-page login-page--single relative isolate grid min-h-screen w-full box-border place-items-center overflow-hidden bg-[radial-gradient(circle_at_18%_18%,rgba(255,190,112,.34),transparent_25rem),radial-gradient(circle_at_82%_82%,rgba(232,75,66,.2),transparent_27rem),linear-gradient(145deg,#fffaf6,#fff1ea)] px-4 py-5 sm:px-5 sm:py-8">
      ...
      <section className="login-panel login-panel--single grid min-h-0 w-full max-w-full box-border place-items-center bg-transparent p-0">
        <div className="login-shell grid w-[calc(100vw-32px)] max-w-[470px] box-border justify-items-stretch overflow-hidden sm:w-full">
          <div className="brand brand--large brand--login mx-auto mb-5 flex items-center gap-2.5 text-2xl font-extrabold tracking-[-0.04em] text-[#3b282c]"><span className="brand__mark grid size-[50px] place-items-center rounded-[13px] bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-lg"><ChefHat /></span><span>Resto<strong className="text-brand-500">Hub</strong></span></div>
          <div className="login-card box-border w-full max-w-full rounded-2xl border border-[#e5d2cb]/90 bg-white/90 p-4 shadow-[0_28px_70px_rgba(101,54,46,.14)] backdrop-blur-xl sm:p-8">
            <div className="login-card__heading mb-5 text-center"><span className="eyebrow text-[0.68rem] font-extrabold uppercase tracking-[0.12em] text-brand-600">Welcome back</span><h2 className="mb-2 mt-2 text-[2rem] font-extrabold tracking-[-0.04em] text-[#342326] sm:text-4xl">Sign in</h2><p className="m-0 text-sm text-[#74676a]">Everything you need for today's service.</p></div>
            <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <Button type="submit" size="lg" loading={isSubmitting} className="login-submit mt-1 w-full">Sign in</Button>
            </form>
            <p className="login-card__help mx-auto mb-0 mt-5 max-w-sm text-center text-xs leading-relaxed text-[#74676a]">Need help? Ask your manager.</p>
          </div>
          <div className="login-capabilities mt-3 flex flex-wrap justify-center gap-2" aria-label="RestoHub features">
